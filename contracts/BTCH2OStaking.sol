...
/// @title BTCH2O Staking Contract
/// @notice Staking para el token BITCOIN2 (BTCH2O) con APR y comisiones ajustables por el owner
...
contract BTCH2OStaking {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "BTCH2O: not owner");
        _;
    }

    // ── Constructor ───────────────────────────────────────────────────────────

    /// @param _token    Dirección del token BTCH2O
    /// @param _aprBps   APR inicial en bps (ej. 1200 = 12%)
    constructor(address _token, uint256 _aprBps) {
        require(_token != address(0), "BTCH2O: zero token");
        owner       = msg.sender;
    ...
    /// @notice Depositar (stake) tokens BTCH2O
    function stake(uint256 amount) external {
        require(amount > 0, "BTCH2O: zero amount");
        _accrueRewards(msg.sender);

        uint256 fee       = (amount * depositFeeBps) / 10_000;
        uint256 netAmount = amount - fee;

        require(stakingToken.transferFrom(msg.sender, address(this), amount), "BTCH2O: transferFrom failed");
    ...
    /// @notice Retirar (unstake) tokens BTCH2O
    function unstake(uint256 amount) external {
        UserInfo storage u = _users[msg.sender];
        require(amount > 0,           "BTCH2O: zero amount");
        require(u.staked >= amount,   "BTCH2O: insufficient stake");
        ...
        require(stakingToken.transfer(msg.sender, netAmount), "BTCH2O: transfer failed");
    ...
    function claim() external {
        ...
        require(reward > 0, "BTCH2O: nothing to claim");
        ...
        require(
            ...
            "BTCH2O: insufficient reward balance - owner must fund the contract"
        );

        if (fee > 0) feeBalance += fee;
        u.pendingReward = 0;

        require(stakingToken.transfer(msg.sender, netReward), "BTCH2O: transfer failed");
    ...
    /// @notice Cambiar el APR (en bps). 100 = 1%, 1200 = 12%.
    function setApr(uint256 _aprBps) external onlyOwner {
        require(_aprBps <= 100_000, "BTCH2O: APR demasiado alto");
        aprBps = _aprBps;
        emit AprSet(_aprBps);
    }
    ...
    function setFees(uint256 _deposit, uint256 _withdraw, uint256 _claim) external onlyOwner {
        require(_deposit  <= 2_000, "BTCH2O: deposit fee max 20%");
        require(_withdraw <= 2_000, "BTCH2O: withdraw fee max 20%");
        require(_claim    <= 2_000, "BTCH2O: claim fee max 20%");
    ...
    function withdrawFees() external onlyOwner {
        ...
        require(amount > 0, "BTCH2O: no fees to withdraw");
        feeBalance = 0;
        require(stakingToken.transfer(owner, amount), "BTCH2O: transfer failed");
        emit FeeWithdrawn(owner, amount);
    }

    /// @notice Transferir la propiedad del contrato
    function transferOwnership(address _newOwner) external onlyOwner {
        require(_newOwner != address(0), "BTCH2O: zero address");
    ...
    function rescueToken(address token, uint256 amount) external onlyOwner {
        require(
            ...
            "BTCH2O: cannot rescue staking token while users are staked"
        );
